...
contract Marketplace {
    ...
    **/
    ...
    function getAvailableMarketItems() public view returns (SellList[] memory){
        SellList[] memory resultTemp = new SellList[](salesId);
        uint index = 0;
        for (uint i=0;i<salesId;i++){
            if(!sales[i].isSold){
                resultTemp[index] = sales[i];
                index ++;
            }
        }
        SellList[] memory result = new SellList[](index);
    ...
    function getSoldMarketItems() public view returns (SellList[] memory){
        SellList[] memory resultTemp = new SellList[](salesId);
        uint index = 0;
        for (uint i=0;i<salesId;i++){
            if(sales[i].isSold){
    ...
    function getSell(uint256 sellId) public view returns (SellList memory){
        return sales[sellId];
    }

    function sellIdfromTokenId(uint256 tokenId) public view returns (uint256){
        for (uint i=0; i<=salesId; i++) {
            if (sales[i].tokenId == tokenId) return i;
        }
    }

    /** 
        @param _token This is the address of the ERC721 token.
        @param _tokenId This is the ID of the token that's inside of the ERC721 token.
    ...
    **/
    function buyListToken(
        ...
        */
        IERC721(sales[_sellId].token).safeTransferFrom(
    ...
    **/
    function transfer(
        ...
        /* 
            Send ERC721 token to _receiver wallet
